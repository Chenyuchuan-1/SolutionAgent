...
本文档面向使用者、方案顾问、产品运营和项目交付人员，说明如何启动和使用 Solution Agent，如何配置后端 `.env`，如何理解生成出来的解决方案 Markdown，图片提示词从哪里来，以及如何按项目需要修改提示词工程。

Solution Agent 的核心流程是：
...
这个文件可以用来检查“每篇文献是否都进入了最终大纲生成上下文”。
...
- `OPENAI_API_KEY`：文本模型、DeepSeek 总结模型、图片模型可共用的主 key。
- `OPENAI_TEXT_MODEL`：生成解决方案大纲的模型，当前用于 GPT-5.5。
- `OPENAI_TEXT_URL`：OpenAI 兼容的 chat completions 地址。可以填完整 `/chat/completions`，也可以填 `/v1` 基础地址。
...
get_solution_system_prompt(language)
```
...
```python
get_outline_generation_prompt(language)
```
...
```python
get_image_style_lock_prompt(language)
get_single_topic_image_prompt_template(language)
```
...
```python
get_image_edit_prompt(language)
```
...
- 统一导出 prompts 中的函数和常量。
- 业务代码通过这里导入提示词函数。

一般不需要修改，除非新增了新的 prompt 文件。
...
outline_prompts.py
-> get_outline_generation_prompt(language)
-> format(domain, goal, scenario, user_prompt, parsed_documents, retrieval_results, topic_count)
-> user_prompt

OpenAIClient.generate_outline(system_prompt, user_prompt)
-> GPT-5.5
-> outline.md
```
...
```text
app/services/outline_service.py -> generate_outline()
app/models/openai_client.py -> generate_outline()
```
...
-> 每篇 PDF 构造 system_prompt + user_prompt
-> OpenAIClient.summarize_with_deepseek()
-> parsed_documents_summary.json
-> parsed_documents_outline_context.md
-> outline prompt
```

每篇文档的 DeepSeek 总结要求包括：

- 文档核心结论
- 与用户目标相关的可用依据
- 可转化为解决方案的做法/模块/流程
- 对图片脚本有价值的视觉元素
- 需要人工核验的数字、实验结论或企业化假设
...
-> 每 5 条构造一个 batch
-> DeepSeek 批次归纳
-> search_results_summary.json
-> search_results_outline_context.md
-> outline prompt
```

每个 batch 的 DeepSeek 总结要求包括：
...
```text
当前 Markdown
-> 第 11 节 Detailed Solution Drawing Prompts
-> _topics_from_drawing_plan()
-> P01/P02/P03...
-> _drawing_style_lock()
-> 第 11 节统一风格锁定
-> image_prompts.py 通用图片模板
-> 每张图最终 image_prompt
-> OpenAIClient.generate_image()
```

真实送给图片模型的 prompt 保存到：
...
当前代码已经改为每篇文献独立预算，避免第一篇长总结挤掉后续文献。重新生成大纲后，所有上传文献都应进入 text LLM。
...
每 5 条为一个 batch
不限制 batch 数
每个 batch 的 DeepSeek 归纳最多保留约 10000 字符
```
...
```text
prompts/topic_prompts.json
meta.json -> images -> generation_mode
```

如果 `generation_mode` 是 `model`，说明真实调用了图片模型。

如果是 `mock_svg`，说明走了 mock fallback。默认 `MOCK_IMAGE_FALLBACK=false`，不应静默 mock。

### 11.7 为什么前端显示历史记录已有图片，不能重新生成？
...
- `.env` 中没有错误 key 或空 key。
- PDF 解析状态为 `parsed`。
- 每篇上传文献都进入 `parsed_documents_outline_context.md`。
- 每组检索都进入 `search_results_outline_context.md`。
- 大纲第 4 节证据地图列出了关键来源。
- 大纲第 6 节 topic 编号连续。
- 大纲第 11 节 P01-Pxx 与 T01-Txx 一一对应。
- 图片 `parsed_drawing_topics.json` 解析正确。
- 图片 `topic_prompts.json` 不包含“需要核验、风险提示、信息来源、下一步计划”等不该出现在画面里的文字。
- 图片不包含真实公司 Logo、未经核验数字、政策编号或标准编号。
- 下载 MD、HTML、图片 ZIP 均可用。
